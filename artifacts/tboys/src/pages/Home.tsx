...
import { useListProducts, useGetSiteContent } from "@workspace/api-client-react";
...
export default function Home() {
  ...
  const headline = site?.heroHeadline ?? "The New Classic";
  const subheading = site?.heroSubheading ?? "Unmistakably modern. Confident and refined. Step into the atelier.";
  const ctaPrimary = site?.heroCtaPrimary ?? "SHOP NOW";
  const ctaSecondary = site?.heroCtaSecondary ?? "EXPLORE COLLECTION";

  return (
    <div className="min-h-screen">
      <section className="relative h-screen w-full flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0 w-full h-full bg-secondary">
          <img src="/images/hero-1.png" alt="TBOY'S Luxury Collection" className="w-full h-full object-cover object-center" />
        ...
        <div className="relative z-10 text-center text-white px-4 flex flex-col items-center animate-in fade-in slide-in-from-bottom-8 duration-1000">
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
            ...
            <Button asChild size="lg" variant="outline" className="rounded-none px-8 py-6 text-sm tracking-widest bg-transparent text-white border-white hover:bg-white/10 hover:text-white">
              <Link href="/collections">{ctaSecondary}</Link>
            </Button>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4 md:px-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-0 items-center">
            <div className="aspect-[3/4] md:aspect-auto md:h-[700px] w-full bg-secondary md:pr-12">
              <img src="/images/hero-2.png" alt="Premium fabric details" className="w-full h-full object-cover" />
            </div>
